...
class Supervisor extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 1000) {
      console.log(`Supervisor aprobó la solicitud de $${amount}.`);
      return;
    }
...
class Manager extends BaseApprover {
  ...
  override approveRequest(amount: number): void {
    if (amount <= 5000) {
      console.log(`Manager aprobó la solicitud de $${amount}.`);
      return;
    }
...
class Director extends BaseApprover {
  ...
  // TODO: Si el monto es mayor a 10000, pasar la solicitud al siguiente aprobador

  override approveRequest(amount: number): void {
    if (amount <= 10000) {
      console.log(`Director aprobó la solicitud de $${amount}.`);
      return;
    }
