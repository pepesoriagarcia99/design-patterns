...
class Airplane {
  ...
  sendMessageToAirplane(to: Airplane, message: string): void {
    console.log(`${this.getId()} send to ${to.getId()} message: "${message}"`);

    this.controlTower.sendMessage(this, message, to);
  }
}
...
class ControlTower {
  ...
  sendMessage(sender: Airplane, message: string, to?: Airplane): void {
    if (!to) {
      this.airplanes
        .filter((a) => a !== sender)
        .forEach((a) => a.receiveMessage(sender, message));

      return;
    }

    const targetAirplane = this.airplanes.find((a) => a === to);

    if (!targetAirplane) {
      console.log("Unknown airplane");
      return;
    }

    targetAirplane.receiveMessage(sender, message);
  }
...
function main(): void {
  ...
  airplane2.sendMessageToAirplane(airplane1, "buen vuelo 101");
}
